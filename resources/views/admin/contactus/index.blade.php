        ...
        <div class="kt-portlet kt-portlet--mobile">
                ...
                {{-- <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-wrapper">
                        <div class="kt-portlet__head-actions">
                            <a href="{{ route('master-contactus.create') }}" class="btn btn-brand btn-elevate btn-icon-sm">
                                <i class="la la-plus"></i> Create a new contact
                            </a>
                        </div>  
                    </div>      
                </div> --}}
            ...
            <div class="kt-portlet__body">
                ...
                <table class="table table-striped- table-bordered table-hover table-checkable simple-datatable">
                    ...
                    <tbody>
                        @foreach ($recContactUs as $index => $row)
                        <tr>
                            <td>{{ ++$index }}</td>
                            <td>
                                <a href="{{ route('master-contactus.edit',['id'=>$row->id]) }}">{{ $row->name }}</a>
                                {{-- {{ $row->name }} --}}
                            </td>
                            ...
                            <td>{{ $row->subject }}</td>
                            <td>
                                {!! ($row->is_spam=='1')?'<span class="kt-badge kt-badge--brand kt-badge--inline kt-badge--pill" style="background: red;">Yes</span>':'<span class="kt-badge kt-badge--brand kt-danger--inline kt-badge--pill">No</span>' !!}
                            </td>
                            <td>{{ date_format(date_create($row->updated_at),"d M Y") }}</td>
                            <td>
                                <span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill delete-link" data-id="{{ $row->id }}" data-link="administrator/master-contactus">
