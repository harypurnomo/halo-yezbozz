    ...
    <div class="row">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ route('master-banner.update',['id'=>$recBannerByID->id]) }}" method="post" enctype="multipart/form-data">
                    {{method_field('PUT')}}
                    {{ csrf_field() }}
                    <div class="kt-portlet__body">
                        @if ($errors->any())
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="title_en" class="form-control summernote" required="">{{ $recBannerByID->title_en }}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Title ID<span class="kt-font-danger">*</span></label>
                            <textarea name="title_id" class="form-control summernote" required="">{{ $recBannerByID->title_id }}</textarea>
                        </div>

                        <div class="form-group">
                            <label>Brief EN <span class="kt-font-danger">*</span></label>
                            <textarea name="brief_en" id="brief_en" class="form-control" maxlength="140" required="">{{ $recBannerByID->brief_en }}</textarea>
                            <span id="char-brief_en">140</span> Character(s) Remaining
                        </div>
                        <div class="form-group">
                            <label>Brief ID <span class="kt-font-danger">*</span></label>
                            <textarea name="brief_id" id="brief_id" class="form-control" maxlength="140" required="">{{ $recBannerByID->brief_id }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-4">
                                <div class="kt-avatar kt-avatar--outline" id="kt_user_avatar">
                                    @if($recBannerByID->picture=='')
                                    <div class="kt-avatar__holder" style="background-image: url({{ asset('admin/template/iphub/noimage.png') }});"></div>
                                    @else
                                    <div class="kt-avatar__holder" style="background-image: url({{ url('uploads/banner') }}/{{ $recBannerByID->picture }})"></div>
                                    @endif
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="sort" class="form-control" placeholder="Your Sort Menu" value="{{ $recBannerByID->sort }}" required="">
                        </div>

                        <div class="form-group">
                            <label>Link EN</label>
                            <input type="text" name="link_en" class="form-control" placeholder="Your Link ID" value="{{ $recBannerByID->link_en }}" minlength="5" maxlength="100">
                            <small>If you only have one link use the (Link EN)</small>
                        </div>
                        <div class="form-group">
                            <label>Link ID</label>
                            <input type="text" name="link_id" class="form-control" placeholder="Your Link ID" value="{{ $recBannerByID->link_id }}" minlength="5" maxlength="100">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control col-4" name="is_active" required="">
                                ...
                                <option value="1" {{ ($recBannerByID->is_active==1)?'selected':'' }}>Yes</option>
                                <option value="0" {{ ($recBannerByID->is_active==0)?'selected':'' }}>No</option>
                            </select>
                        </div>
                    </div>
