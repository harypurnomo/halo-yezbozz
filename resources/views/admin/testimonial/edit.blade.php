    ...
    <div class="row">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ route('master-testimonial.update',['id'=>$recTestimonialsByID->id]) }}" method="post" enctype="multipart/form-data">
                    {{method_field('PUT')}}
                    {{ csrf_field() }}
                    <div class="kt-portlet__body">
                        @if ($errors->any())
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="full_name" class="form-control" value="{{ $recTestimonialsByID->full_name }}" placeholder="Your Fullname" required="" minlength="5" maxlength="100">
                        </div>
                        <div class="form-group">
                            <label>Company Name <span class="kt-font-danger">*</span></label>
                            <input type="text" name="company_name" class="form-control" value="{{ $recTestimonialsByID->company_name }}" placeholder="Your Company Name" required="" minlength="5" maxlength="100">
                        </div>
                        <div class="form-group">
                            <label>Testimonial <span class="kt-font-danger">*</span></label>
                            <textarea name="testimonial" class="summernote" id="kt_summernote_1" required="">{{ $recTestimonialsByID->testimonial }}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Pic</label>
                            <div class="col-4">
                                <div class="kt-avatar kt-avatar--outline" id="kt_user_avatar">
                                    @if($recTestimonialsByID->picture=='')
                                    <div class="kt-avatar__holder" style="background-image: url({{ asset('admin/template/iphub/noimage.png') }});"></div>
                                    @else
                                    <div class="kt-avatar__holder" style="background-image: url({{ url('uploads/testimonial') }}/{{ $recTestimonialsByID->picture }})"></div>
                                    @endif
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control col-4" name="star" required>
                                ...
                                @for($a=1; $a<=5; $a++)
                                <option value="{{ $a }}" {{ ($recTestimonialsByID->star==$a)?'selected':'' }}>{{ $a }}</option>
                                @endfor
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control col-4" name="is_active" required="">
                                <option value="">-- choose one of them --</option>
                                <option value="1" {{ ($recTestimonialsByID->is_active==1)?'selected':'' }}>Yes</option>
                                <option value="0" {{ ($recTestimonialsByID->is_active==0)?'selected':'' }}>No</option>
                            </select>
                        </div>
                    </div>
