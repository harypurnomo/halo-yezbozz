    ...
    <div class="row">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ route('master-vendor-product-price.update',['id'=>$recVendorProductsPriceByID->id]) }}" method="post" enctype="multipart/form-data">
                    {{method_field('PUT')}}
                    {{ csrf_field() }}
                            ...
                            <div class="tab-pane fade show active p-3" id="general" role="tabpanel" aria-labelledby="tab-general">
                                <div class="form-group">
                                    ...
                                    <select class="form-control col-4 kt-select2-general" name="product_id" required="">
                                        ...
                                        <option value="{{ $item->id }}" {{ ($item->id==$recVendorProductsPriceByID->product_id)?'selected':'' }}>{{ $item->product_title_id }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="form-control col-4 kt-select2-general" name="vendor_id" required="">
                                        ...
                                        <option value="{{ $item->id }}" {{ ($item->id==$recVendorProductsPriceByID->vendor_id)?'selected':'' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Note <span class="kt-font-danger">*</span></label>
                                    <input type="text" name="note" id="name" class="form-control" placeholder="{{ Library::modules(Request::segment(1).'/'.Request::segment(2))[0]->module_name }} Note" value="{{ $recVendorProductsPriceByID->note }}" required="" minlength="3" maxlength="100">
                                    <span id="char-name">100</span> Character(s) Remaining
                                </div>
                                <div class="form-group">
                                    <label>External Link</label>
                                    <input type="text" name="external_link" class="form-control" placeholder="Your External Link" value="{{ $recVendorProductsPriceByID->external_link }}">
                                </div>
                                                            
                                <div class="form-group">
                                    <label>Qty <span class="kt-font-danger">*</span></label>
                                    <input type="number" name="qty" class="form-control" required placeholder="Your {{ Library::modules(Request::segment(1).'/'.Request::segment(2))[0]->module_name }} Qty" value="{{ $recVendorProductsPriceByID->qty }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="price" class="form-control input-numeral" required placeholder="Your {{ Library::modules(Request::segment(1).'/'.Request::segment(2))[0]->module_name }} Price" value="{{ $recVendorProductsPriceByID->price }}">
                                </div>
                            
                                <div class="form-group">
                                    <label>Tax (in percent %)</label>
                                    <input type="text" name="tax" class="form-control tax" placeholder="Your {{ Library::modules(Request::segment(1).'/'.Request::segment(2))[0]->module_name }} Tax" value="{{ $recVendorProductsPriceByID->tax }}">
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control col-4" name="recommended" required="">
                                        <option value="">-- choose one of them --</option>
                                        @for($i = 1; $i <= 10; $i++)
                                        <option value="{{ $i }}" {{ ($i==$recVendorProductsPriceByID->recommended)?'selected':'' }}>{{ $i }}</option>    
                                        @endfor
                                    </select>
                                </div>
                            </div>
