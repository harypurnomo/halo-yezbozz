    ...
    <div class="row">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ route('master-vendor-order.update',['id'=>$recVendorsOrderHistoryByID->id]) }}" method="post" enctype="multipart/form-data">
                    {{method_field('PUT')}}
                    {{ csrf_field() }}
                            ...
                            <div class="tab-pane fade show active p-3" id="general" role="tabpanel" aria-labelledby="tab-general">
                                <div class="form-group">
                                    ...
                                    <select class="form-control col-4" name="vendor_id" required="">
                                        ...
                                        <option value="{{ $item->id }}" {{ ($item->id==$recVendorsOrderHistoryByID->vendor_id)?'selected':'' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Name <span class="kt-font-danger">*</span></label>
                                    <input type="text" name="name" id="name" class="form-control" placeholder="{{ Library::modules(Request::segment(1).'/'.Request::segment(2))[0]->module_name }} Name" value="{{ $recVendorsOrderHistoryByID->name }}" required="" minlength="3" maxlength="100">
                                    <span id="char-name">100</span> Character(s) Remaining
                                </div>
                                <div class="form-group">
                                    <label>File Attachement</label><br>
                                    @if($recVendorsOrderHistoryByID->file_attachement=='')
                                    <a href="javascript:;">Not Available</a>
                                    @else
                                    <a href="{{ url('uploads/vendor/order') }}/{{ $recVendorsOrderHistoryByID->file_attachement }}" target="_blank">{{ $recVendorsOrderHistoryByID->file_attachement }}</a>
                                    <a href="javascript:;" style="float: right;" class="delete-file-link" data-link="{{ route('master.vendor.order.delete',['id'=>$recVendorsOrderHistoryByID->id]) }}"><i class="fa fa-trash"></i> Remove file</a>
                                    @endif
