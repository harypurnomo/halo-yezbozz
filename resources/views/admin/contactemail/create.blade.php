	...
	<div class="row">
        <div class="kt-portlet kt-portlet--mobile">
            ...
            <div class="kt-portlet__body">
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ route('master-contact-email.store') }}" method="post" enctype="multipart/form-data">
                	{{ csrf_field() }}
                    <div class="kt-portlet__body">
                    	@if ($errors->any())
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control col-4" name="groups_announcement_id" required="">
                                ...
                                @foreach ($recGroupsAnnouncement as $item)
                                    <option value="{{ $item->id }}" {{ (old('groups_announcement_id')==$item->id)?'selected':'' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
